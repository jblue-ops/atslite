...
RSpec.configure do |config|
  ...
  config.before(:each, external_api: true) do
    VCR.insert_cassette(
  ...
  config.after(:each, external_api: true) do
    VCR.eject_cassette
  end
  
  # Helper to stub external service calls
  config.before(:each, stub_external: true) do
...
module VCRHelpers
  ...
  def record_vcr_cassette(name, &block)
