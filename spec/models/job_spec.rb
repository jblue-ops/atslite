...
RSpec.describe Job, type: :model do
  ...
  describe "rich text associations" do
    it "has rich text for description" do
      job = create(:job, organization: organization, hiring_manager: hiring_manager)
      expect(job).to respond_to(:description)
      expect(job.description).to be_an(ActionText::RichText)
    end

    it "has rich text for requirements" do
      job = create(:job, organization: organization, hiring_manager: hiring_manager)
      expect(job).to respond_to(:requirements)
      expect(job.requirements).to be_an(ActionText::RichText)
    end

    it "has rich text for qualifications" do
      job = create(:job, organization: organization, hiring_manager: hiring_manager)
      expect(job).to respond_to(:qualifications)
      expect(job.qualifications).to be_an(ActionText::RichText)
    end

    it "has rich text for benefits" do
      job = create(:job, organization: organization, hiring_manager: hiring_manager)
      expect(job).to respond_to(:benefits)
      expect(job.benefits).to be_an(ActionText::RichText)
    end

    it "has rich text for application_instructions" do
      job = create(:job, organization: organization, hiring_manager: hiring_manager)
      expect(job).to respond_to(:application_instructions)
      expect(job.application_instructions).to be_an(ActionText::RichText)
    end
  end
  ...
  describe "class methods" do
    describe ".search" do
      it "searches by title" do
        job1 = create(:job, title: "Ruby Developer", organization: organization, hiring_manager: hiring_manager)
        expect(described_class.search("Ruby")).to contain_exactly(job1)
      end

      it "searches by location" do
        job_with_location = create(:job, location: "San Francisco", organization: organization, hiring_manager: hiring_manager)
        expect(described_class.search("Francisco")).to contain_exactly(job_with_location)
      end
    end
  end
