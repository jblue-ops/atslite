...
RSpec.describe JobPolicy, type: :policy do
  ...
  describe "#index?" do
    subject { described_class.new(user, Job).index? }
  ...
  describe "#show?" do
    subject { described_class.new(user, job).show? }
  ...
  describe "#create?" do
    subject { described_class.new(user, job).create? }
  ...
  describe "#new?" do
    subject { described_class.new(user, job).new? }

    it "delegates to create?" do
      expect(subject).to eq(described_class.new(user, job).create?)
    end
  end

  describe "#update?" do
    subject { described_class.new(user, job).update? }
  ...
  describe "#edit?" do
    subject { described_class.new(user, job).edit? }

    it "delegates to update?" do
      expect(subject).to eq(described_class.new(user, job).update?)
    end
  end

  describe "#destroy?" do
    subject { described_class.new(user, job).destroy? }
  ...
  describe "#publish?" do
    subject { described_class.new(user, draft_job).publish? }
    ...
    context "when job cannot be published" do
      subject { described_class.new(user, invalid_job).publish? }
  ...
  describe "#close?" do
    subject { described_class.new(user, published_job).close? }
    ...
    context "when job is not published" do
      subject { described_class.new(user, draft_job).close? }
  ...
  describe "#reopen?" do
    subject { described_class.new(user, closed_job).reopen? }
    ...
    context "when job is not closed" do
      subject { described_class.new(user, draft_job).reopen? }
  ...
  describe "#archive?" do
    subject { described_class.new(user, job).archive? }
  ...
  describe "#unarchive?" do
    subject { described_class.new(user, archived_job).unarchive? }
    ...
    context "when job is not archived" do
      subject { described_class.new(user, job).unarchive? }
  ...
  describe "#view_analytics?" do
    subject { described_class.new(user, job).view_analytics? }
  ...
  describe "#view_applications?" do
    subject { described_class.new(user, job).view_applications? }
  ...
  describe "#manage_applications?" do
    subject { described_class.new(user, job).manage_applications? }
